...
class ST_OverlapsTest extends AbstractFunctionalTestCase
{
    ...
    {
        $query = $this->_getEntityManager()->createQuery('SELECT ST_Overlaps(ST_Buffer(ST_GeomFromText(\'POINT(1 0.5)\'), 3), ST_Buffer(ST_GeomFromText(\'LINESTRING(1 0, 1 1, 3 5)\'), 0.5)) AS value FROM Jsor\\Doctrine\\PostGIS\\Test\\fixtures\\PointsEntity point');
